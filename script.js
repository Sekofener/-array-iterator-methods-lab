...
isAdultPresent = devs.some((anyperson) => {
return ( 2024 - anyperson.year) >= 18 
})
...
isEveryone19OrOlder = devs.every((everyperson) => {
    return (2024 - everyperson.year) >= 19
...
travelMethodCounts = travelMethods.reduce(function(acc, vote){
    if(acc[vote]){
      acc[vote] = acc[vote] + 1;
    } else {
      acc[vote] = 1;
